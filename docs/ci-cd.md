...
6. `npm run build-storybook` — Storybook build

> This is the JavaScript branch, so there is no type-check step. Coverage is available locally via `npm run test:cov`; CI runs `test:unit` for speed.
