...
This template uses **file-based routing**, built into **Vue Router 5** (no extra plugin). Every file under `src/pages/` becomes a route, and route types are generated automatically into `src/typed-router.d.ts`.
...
2. Run the dev server (`npm run dev`) or a build (`npm run build:prod`). Vite regenerates `src/typed-router.d.ts` so `RouterLink`, `useRoute`, and `useRouter` stay type-safe.
...
1. **Remove the file-based plugin from Vite.** In `vite.config.ts`, delete the `VueRouter` plugin import and its entry in `plugins` (it must be registered **before** `vue()`).

2. **Define routes by hand** in `src/plugins/router.ts`. Replace the auto-generated routes with your own array:
...
3. **Drop the generated artifacts.** Delete `src/typed-router.d.ts` and remove the `vue-router/auto-routes` import. You can keep the `src/pages/` folder or reorganize components however you like.
