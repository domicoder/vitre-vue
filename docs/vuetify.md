...
This branch adds [Vuetify](https://vuetifyjs.com/) on top of the base template, alongside Tailwind CSS 4.
...
- Plugin: [`src/plugins/vuetify.ts`](../src/plugins/vuetify.ts) creating the Vuetify instance and importing `vuetify/styles` + the MDI font CSS.
- Vite: `vuetify({ autoImport: true })` registered in [`vite.config.ts`](../vite.config.ts).
- App: `app.use(vuetify)` in [`src/app/main.ts`](../src/app/main.ts) and a `<v-app>` wrapper in [`src/app/App.vue`](../src/app/App.vue).
